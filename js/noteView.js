    ...
    var init = function () {
        ...
        board.addEventListener("focusin", function (event) {
            ...
            if (target.classList.contains("note-title")) {
                target.select();
                target.focused = "true";
                target.parentNode.parentNode.setAttribute("draggable", false);
            }
        });
        ...
        board.addEventListener("focusout", function (event) {
            var parentContent = event.target.parentNode,
                ...
                data = {
                    ...
                    title: parentContent.children[0].value,
                    message: parentContent.children[1].value,
                    editionDate: (new Date()).toLocaleString('en-US', { hour12: false })
                }

            if (event.target.classList.contains("note-title") || event.target.classList.contains("note-content")) {
                event.target.focused = "false";
                parentContent.parentNode.setAttribute("draggable", true);
            }
        ...
        board.addEventListener("drop", function (event) {
            ...
            if (data.guid && data.guid != noteReference.guid) {
        ...
        createNoteElem = function (note) {
            var doc = board.ownerDocument || board,
                ...
                noteElemString = '<div class="note note-filled" draggable="true">' +
                    ...
                    '<textarea class="note-title" placeholder="Title"></textarea>' +
                    '<textarea class="note-content" placeholder="Message"></textarea>' +
            ...
            fragment.children[0].children[1].children[0].value = note.title;
